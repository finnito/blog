...
There is a new, although [not particularly well supported][1] attribute for `<iframe>` and `<img/>` tags: `loading="lazy/eager/auto"` - this is huge because it allows the browser to determine whether an image or iframe should be loaded as the user proceeds through a webpage. No more JS - just a sweet native progressive enhancement.
